...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'resend_verification') {
    ...
    try {
        error_log("Resend verification POST: " . print_r($_POST, true));
        $subscriberId = $_POST['subscriber_id'] ?? 0;
...
// Add modal functions to head so they load before Alpine
$additionalHead = '<script>
const csrfToken = \'' . generateCSRFToken() . '\';
window.saveLists = window.verifySubscriber = window.confirmDelete = window.confirmResend = window.resendVerification = window.bulkResend = null; // Will be defined after Alpine loads
</script>';
?>
<?php include __DIR__ . '/includes/header.php'; ?>

<script>
    // Define modal functions after DOM loads but reference them globally
    document.addEventListener('DOMContentLoaded', function () {
        window.saveLists = function (event) {
            const component = Alpine.$data(document.querySelector('#subscribers-container'));
            ...
            if (component.isBulkChange) {
                if (component.selectedIds.length === 0 || selectedListIds.length === 0) {
                ...
                selectedListIds.forEach(listId => {
                    fetch('subscribers.php', {
                        ...
                        body: 'action=bulk_add_to_list&list_id=' + listId + '&subscriber_ids=' + JSON.stringify(component.selectedIds) + '&csrf_token=' + encodeURIComponent(csrfToken)
                    })
                        .then(response => response.json())
                        .then(data => {
                            if (++completed === selectedListIds.length) {
                ...
                fetch('subscribers.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: formData })
                    .then(response => response.json())
                    .then(data => {
                        component.showChangeListModal = false;
                        setTimeout(() => window.location.reload(), 500);
        ...
        window.verifySubscriber = function (event) {
            const component = Alpine.$data(document.querySelector('#subscribers-container'));
            component.showVerifyConfirm = false;
            fetch('subscribers.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: 'action=verify&subscriber_id=' + component.verifyingId + '&csrf_token=' + encodeURIComponent(csrfToken)
            })
                .then(response => response.json())
                .then(data => { if (data.success) setTimeout(() => window.location.reload(), 500); })
                .catch(console.error);
        };
        ...
        window.confirmDelete = function (event) {
            ...
            fetch('subscribers.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: 'action=bulk_delete&subscriber_ids=' + JSON.stringify(component.selectedIds) + '&csrf_token=' + encodeURIComponent(csrfToken)
            })
                .then(response => response.json())
                .then(data => { if (data.success) setTimeout(() => window.location.reload(), 500); })
                .catch(console.error);
        ...
        window.resendVerification = function (subscriberId, email) {
            const component = Alpine.$data(document.querySelector('#subscribers-container'));
            component.resendIsBulk = false;
            component.resendSingleId = subscriberId;
            component.resendSingleEmail = email;
            component.showResendConfirm = true;
        };

        window.bulkResend = function () {
            const component = Alpine.$data(document.querySelector('#subscribers-container'));
            if (component.selectedIds.length === 0) return;
            component.resendIsBulk = true;
            component.showResendConfirm = true;
        };

        window.confirmResend = function () {
            console.log('confirmResend called');
            try {
                const component = Alpine.$data(document.querySelector('#subscribers-container'));
                component.showResendConfirm = false;

                let body = '';
                if (component.resendIsBulk) {
                    body = 'action=bulk_resend&subscriber_ids=' + JSON.stringify(component.selectedIds);
                } else {
                    console.log('Single resend ID:', component.resendSingleId);
                    body = 'action=resend_verification&subscriber_id=' + component.resendSingleId;
                }
                ...
                console.error('confirmResend Error:', e);
...
<div id="subscribers-container"
    x-data="{ selectedIds: [], showVerifyConfirm: false, verifyingId: null, verifyingEmail: '', showChangeListModal: false, changeListId: null, isBulkChange: false, showDeleteConfirm: false, showResendConfirm: false, resendIsBulk: false, resendSingleId: null, resendSingleEmail: '' }">
    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center space-x-4">
            ...
            <div x-show="selectedIds.length > 0" x-transition class="flex items-center space-x-2">
                ...
                <button
                    @click="isBulkChange = true; showChangeListModal = true; setTimeout(() => document.querySelectorAll('.list-checkbox').forEach(cb => cb.checked = false), 100)"
                    class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition text-sm">
                    Add to List
                </button>
                <button @click="bulkResend()"
                ...
                <?php foreach ($subscribers as $sub): ?>
                    <tr id="subscriber-<?= $sub['id'] ?>" class="hover:bg-gray-50">
                            ...
                            <?php if (!empty($sub['list_names'])): ?>
                                ...
                                <?php foreach ($lists_array as $index => $list): ?>
                                    ...
                                    <?php if ($sub['unsubscribed']): ?>
                                        ...
                                        <span
                                            class="inline-block px-2 py-1 bg-gray-100 text-gray-500 text-xs rounded mr-1 mb-1 line-through"><?= htmlspecialchars($list) ?></span>
                                    ...
                                    <?php else: ?>
                                        <!-- Active subscription -->
                                        <span
                                            class="inline-block px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded mr-1 mb-1"><?= htmlspecialchars($list) ?></span>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <?php if ($sub['unsubscribed']): ?>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            <?= htmlspecialchars($sub['site_name'] ?? 'N/A') ?>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap status-cell">
                            <?php if ($sub['unsubscribed']): ?>
                                <span
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Unsubscribed</span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            <?= date('Y-m-d H:i', strtotime($sub['subscribed_at'])) ?>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex space-x-2">
                                <?php if (!$sub['verified']): ?>
                                    <button
                                        @click="showVerifyConfirm = true; verifyingId = <?= $sub['id'] ?>; verifyingEmail = '<?= htmlspecialchars($sub['email'], ENT_QUOTES) ?>'"
                                        class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition text-xs">
                                        Verify
                                    </button>
                                    <button
                                        @click="resendVerification(<?= $sub['id'] ?>, '<?= htmlspecialchars($sub['email'], ENT_QUOTES) ?>')"
    ...
    <template x-teleport="body">
        <div x-show="showVerifyConfirm" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div @click="showVerifyConfirm = false"
                    class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity">
                </div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">Verify Subscriber</h3>
                    <p class="text-gray-600 mb-6">Are you sure you want to manually verify <strong
                            x-text="verifyingEmail"></strong>?</p>
                    <div class="flex justify-end space-x-3">
                        <button @click="showVerifyConfirm = false"
                            class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                        <button @click="verifySubscriber()"
                            class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Verify</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Change List Modal -->
    <template x-teleport="body">
        <div x-show="showChangeListModal" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div @click="showChangeListModal = false"
                    class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4"
                        x-text="isBulkChange ? 'Add to List' : 'Manage Lists'"></h3>
                    <p class="text-gray-600 mb-4"
                        x-text="isBulkChange ? 'Add ' + selectedIds.length + ' subscriber(s) to:' : 'Select which lists this subscriber belongs to:'">
                    </p>
                    ...
                    <div class="flex justify-end space-x-3">
                        <button @click="showChangeListModal = false"
                            class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                        <button @click="saveLists()"
                            class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition"
                            x-text="isBulkChange ? 'Add to List' : 'Save Lists'"></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <template x-teleport="body">
        <div x-show="showResendConfirm" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div @click="showResendConfirm = false"
                    class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity">
                </div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 text-center mb-2">Resend Verification</h3>
                    <p class="text-gray-600 text-center mb-6">
                        <span x-show="resendIsBulk">
                            Are you sure you want to resend verification emails to <strong
                                x-text="selectedIds.length"></strong> subscriber(s)?
                        </span>
                        <span x-show="!resendIsBulk">
                            Are you sure you want to resend verification email to <strong
                                x-text="resendSingleEmail"></strong>?
                        </span>
                    </p>
                    <div class="flex justify-end space-x-3">
                        <button @click="showResendConfirm = false"
                            class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                        <button @click="confirmResend()"
                            class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Resend
                            Email(s)</button>
                    </div>
                </div>
            </div>
