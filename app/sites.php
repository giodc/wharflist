...
<div x-data="{ showCreateModal: false, showEmbedModal: false, embedCode: '' }">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <button @click="showCreateModal = true" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?= htmlspecialchars($site['name']) ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($site['domain']) ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= htmlspecialchars($site['list_name']) ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600"><?= $site['subscriber_count'] ?></td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                        <button @click="embedCode = `<!-- WharfList Subscription Form -->\n<div id='wharflist-form'></div>\n<script>\n(function() {\n    var script = document.createElement('script');\n    script.src = '<?= BASE_URL ?>/widget.js';\n    script.setAttribute('data-api-key', '<?= htmlspecialchars($site['api_key'], ENT_QUOTES) ?>');\n    script.setAttribute('data-site-id', '<?= $site['id'] ?>');\n    document.head.appendChild(script);\n})();\n<\/script>`; showEmbedModal = true" 
    ...
    <div x-show="showCreateModal" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            <div @click="showCreateModal = false" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
            <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                ...
                <form method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="domain" placeholder="example.com" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Custom Fields (comma separated)</label>
                        <input type="text" name="custom_fields" placeholder="name,phone" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <p class="text-xs text-gray-500 mt-1">Optional additional fields to collect</p>
                    </div>
                    <div class="flex justify-end space-x-3">
                        <button type="button" @click="showCreateModal = false" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">Cancel</button>
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">Create Site</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div x-show="showEmbedModal" 
         ...
         style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div @click="showEmbedModal = false" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>
            <div class="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6">
                ...
                <textarea x-model="embedCode" id="embed-code" readonly class="w-full px-4 py-3 border border-gray-300 rounded-lg font-mono text-sm bg-gray-50" rows="10"></textarea>
                <button @click="navigator.clipboard.writeText(embedCode).then(() => alert('Copied to clipboard!'))" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                    Copy to Clipboard
                </button>
            </div>
        </div>
